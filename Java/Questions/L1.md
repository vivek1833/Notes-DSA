...
1. Constructor injection - using constructor, most common.
...
2. Setter injection - using `set` methods, good when dependencies are not known at compile time.
...
3. Field injection - using `@Autowired` annotation or `@Inject`.
...
- Optimistic locking is a concurrency control mechanism used in database transactions to prevent concurrent updates to a shared resource from overwriting each other's changes.
- It works by using a version number or timestamp to detect and resolve conflicts between concurrent transactions.
...
class PersonService {
    public Person update(Person person) {
        ...
        if (existingPerson.getVersion() != person.getVersion()) {
...
### 53. Load Balancer Algorithm (Random with Two Choices (Power of Two))

- Pick two random servers → send request to the less loaded one.
- This is industry standard for large-scale systems.
- Better performance than round robin with minimal complexity.
- The probability of consistently missing servers becomes vanishingly small extremely quickly.
...
| 3 misses           | 0.125 (12.5%)          |
| 5 misses           | 0.031 (3.1%)           |
| 10 misses          | 0.00098 (0.098%)       |
| 20 misses          | 0.00000095 (0.000095%) |

---

### 54. Conditional Configuration

- `@ConditionalOnProperty` checks if a specific property is present in the application properties file.
- `@ConditionalOnClass` checks if a specific class is present on the classpath.
- `@ConditionalOnBean` checks if a specific bean is present in the application context.
- `@ConditionalOnMissingBean` checks if a specific bean is not present in the application context.

```java
@Configuration
@ConditionalOnProperty(name = "my.property", matchIfMissing = true)
